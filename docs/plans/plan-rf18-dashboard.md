...
- Revisar `ISocioRepository` (ya tiene `CountActivosByUnidadAsync`), `ICuotaRepository` (tiene `GetCuotasPendientesDeTodosLosSociosAsync(Guid?)`; falta "pagadas del mes"), **`IHorarioClaseRepository`** y **`IInscripcionClaseRepository`** (nombres reales; `GetConteoActivasPorHorariosAsync` es reusable para inscriptos) y agregar SOLO lo que falte para: counts de cuotas — **próximas a vencer = Pendiente con vencimiento en [hoy, hoy+5] (ventana explícita, NO reusar `GetCuotasParaRecordatorioAsync` que matchea días exactos)**, vencidas = Pendiente con fechaVencimiento < hoy, pagadas del mes —, horarios del día de la semana actual con inscriptos (mapear `DateTime.DayOfWeek`→`DiaSemana`; usar fecha UTC consistente con los jobs), últimas 10 inscripciones, e inscripciones por día (últimos 7 días). Con `Guid? unidadId` o lista de unidades donde aplique.
...
- `Application/UseCases/Dashboard/`: DTO como en el spec §2; `ExecuteAsync(Guid? unidadId, IReadOnlyCollection<Guid>? unidadesPermitidas)`; si `unidadId` no está en las permitidas → `UnauthorizedAccessException`; `unidadesPermitidas == null` = todas (admin); **colección VACÍA = dueño sin unidades → métricas en cero (distinto de null)**. Registrar DI.
...
- `GET /api/dashboard/stream?unidadId=` → SSE: `Response.ContentType = "text/event-stream"`, headers no-cache + `X-Accel-Buffering: no`, loop `while (!ct.IsCancellationRequested)`: snapshot → serializar → si difiere del último enviado escribir `data: {json}\n\n` sino `: ping\n\n` → `FlushAsync` → `Task.Delay(10s, ct)`. La comparación "cambió o no" en un helper puro testeable. Primer snapshot SIEMPRE se envía.
- ⚠️ **Correcciones del review (obligatorias):**
  1. **camelCase en el stream:** `JsonSerializer.Serialize()` manual default a PascalCase (el camelCase de la API lo pone MVC). Inyectar `IOptions<Microsoft.AspNetCore.Mvc.JsonOptions>` y serializar con `.Value.JsonSerializerOptions` para que el stream matchee el snapshot.
  2. **Desconexión del cliente:** envolver el loop en `try/catch (OperationCanceledException)` — `Task.Delay(10s, ct)` lanza al cerrar la pestaña y el `UseExceptionHandler` global no puede reescribir una respuesta ya iniciada (quedaría una excepción logueada por cada cierre).
  3. **403, no 500:** ambos endpoints (snapshot y stream ANTES de iniciar el loop) capturan `UnauthorizedAccessException` → `Forbid()` (convención del repo: PagosController.cs:45, EmpleadosController.cs).
- TDD: attrs por reflexión + snapshot endpoint con mocks (incl. unidad no permitida → 403); helper de diff. Commit: `feat(dashboard): endpoints snapshot y stream SSE (#RF-18)`
...
- **`frontend/src/types/permisos.ts`**: agregar `'Dashboard'` al union type `Modulo` (sin esto no typechequea nada de lo que sigue).
- **`frontend/src/pages/admin/RolForm.tsx`**: agregar `'Dashboard'` **y también `'Eventos'`** (bug pre-existente: la UI de roles no permite otorgar Eventos) al array `MODULOS` — requerido por RN-16 (el permiso debe ser otorgable a roles custom desde la UI).
- `App.tsx`: `/admin/dashboard` → `DashboardPage` (borrar el redirect viejo `dashboard→socios`). Index de `/admin`: componente chico que mira los permisos del AuthContext → `Navigate` a `dashboard` si tiene Dashboard-Lectura, sino `socios`.
- **`frontend/src/components/layout/Sidebar.tsx`** (ahí vive el nav, NO en AdminLayout): ya existe un link "Inicio" **sin gatear** a `/admin/dashboard` (~L173-184) — gatearlo por el permiso Dashboard-Lectura (o renombrarlo a "Dashboard"), siguiendo cómo se gatean los otros módulos.
...
- Push + `gh pr create --base develop --title "feat(rf18): dashboard en tiempo real multi-espacio (SSE + permisos)"` — body: qué incluye, la migración aditiva de permisos (nota: el seed cartesiano da a Admin las 4 operaciones de Dashboard y la matriz de roles muestra 4 checkboxes de los que solo Lectura tiene efecto — esperado), decisión SSE-por-fetch y fallback, gráfica con selector, landing por permiso, fix de RolForm (Eventos+Dashboard), conteos de tests, y nota de que la base es develop a pedido del usuario. **NO mergear.**
