...
public class CrearEventoCommandTests
{
    ...
    private CrearEventoCommand CrearCommand() =>
        new(_eventoRepo.Object, _unidadRepo.Object, _socioRepo.Object,
    ...
    [Fact]
    public async Task ExecuteAsync_ConSociosActivos_CreaNotificacionesInApp()
    {
        var unidad = new Unidad("Gimnasio Nuevo Malvin", "Malvin, Montevideo");
        var request = new CreateEventoRequest("Torneo de verano", "Torneo abierto",
            DateTime.UtcNow.AddDays(10), unidad.Id);
        ...
        _notificador.Verify(n => n.CrearParaVariosAsync(
            It.Is<IEnumerable<Guid>>(ids =>
                ids.Contains(socios[0].Id) && ids.Contains(socios[1].Id)),
    ...
    [Fact]
    public async Task ExecuteAsync_SinSociosActivos_NoCreaNotificaciones()
    {
        var unidad = new Unidad("Gimnasio Nuevo Malvin", "Malvin, Montevideo");
        var request = new CreateEventoRequest("Torneo de verano", "Torneo abierto",
            DateTime.UtcNow.AddDays(10), unidad.Id);
    ...
    [Fact]
    public async Task ExecuteAsync_NotificadorFalla_ElEventoIgualSeCrea()
    {
        var unidad = new Unidad("Gimnasio Nuevo Malvin", "Malvin, Montevideo");
        var request = new CreateEventoRequest("Torneo de verano", "Torneo abierto",
            DateTime.UtcNow.AddDays(10), unidad.Id);
