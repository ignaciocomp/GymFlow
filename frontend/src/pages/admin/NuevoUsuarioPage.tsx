...
export default function NuevoUsuarioPage() {
  ...
  useEffect(() => {
    listarRoles().then(rs =>
      // El rol "Dueño" solo puede asignarlo el Admin; ocultamos también "Socio" (no es de empleados).
      setRoles(rs.filter(r => r.nombre !== 'Socio' && (esAdmin || r.nombre !== 'Dueño'))),
    )
    unidadesApi.getAll().then(setUnidades)
  }, [esAdmin])
  ...
  const toggleUnidad = (id: string) => {
    setUnidadIds(prev => (prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]))
  }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await crearEmpleado({ ...form, unidadIds })
      ...
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {esRolDueno
              ? 'El Dueño solo verá y gestionará las unidades seleccionadas.'
              : 'Opcional. Restringe al empleado a las unidades seleccionadas.'}
          </p>
        </div>
